...
export function AdminLayout({ children }: AdminLayoutProps) {
  ...
  return (
    <div className="h-screen overflow-hidden flex bg-[var(--color-bg)] text-[var(--color-text-primary)] transition-colors duration-200">
      {sidebarOpen && (
        <div
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 border-r border-[var(--color-border-subtle)] bg-[var(--color-surface)] flex flex-col shrink-0 transform transition-all duration-200 lg:static lg:translate-x-0 ${
          rail ? 'lg:w-[76px]' : 'lg:w-64'
        } ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div
          className={`h-16 px-6 border-b border-[var(--color-border-subtle)] flex items-center justify-between shrink-0 ${
        ...
        >
          ...
          {rail && (
            <div className="hidden lg:block font-bold tracking-tight text-lg">
              E<span className="text-[var(--color-accent)]">S</span>
            </div>
          )}
          <button
            onClick={() => setSidebarOpen(false)}
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="h-16 px-4 sm:px-8 border-b border-[var(--color-border-subtle)] bg-[var(--color-surface)] flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            className="hidden lg:flex p-1.5 rounded-md hover:bg-[var(--color-badge-bg)] text-[var(--color-text-muted)] cursor-pointer"
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 bg-[var(--color-bg-soft)]">{children}</div>
      </main>
    </div>
  );
}
