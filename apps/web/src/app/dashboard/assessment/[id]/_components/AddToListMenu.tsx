  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        title="Add to list"
        className="inline-flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full border border-(--color-border-subtle) text-(--color-text-muted) transition-colors hover:border-(--color-accent) hover:text-(--color-accent) focus:outline-none focus-visible:shadow-[0_0_0_2px_var(--color-bg),0_0_0_4px_var(--color-accent)]"
      >
      ...
      {open && (
        <div className="absolute right-0 z-30 mt-2 w-64 rounded-(--radius-md) border border-(--color-border-subtle) bg-(--color-surface) p-2 shadow-(--shadow-modal)">
          <p className="m-0 px-2 py-1.5 text-xs font-semibold tracking-[0.12em] text-(--color-text-muted) uppercase">
            Add to list
          </p>
              ...
              lists.map((list) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full cursor-pointer items-center justify-between gap-2 rounded-(--radius-sm) px-2 py-2 text-left text-sm text-(--color-text-primary) transition-colors hover:bg-(--color-badge-bg)"
                  >
          ...
          >
            ...
            <button
              ...
              className="inline-flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-(--radius-sm) border border-(--color-border-subtle) text-(--color-text-muted) transition-colors hover:border-(--color-accent) hover:text-(--color-accent) disabled:cursor-not-allowed disabled:opacity-50"
            >
